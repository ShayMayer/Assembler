...
void assemble(char *file_name) {
    ...
    symbol_table *s_table = create_symbol_table() /* the symbol table */, *entry_table, *tmp; /* the entry table */
    memory_cell *ic_memory_table /* the instruction memory table */, *dc_memory_table /* the data memory table */;
    ...
    for (cur_file_info.cur_line_number = 1; !feof(asm_file) && fgets(cur_line, MAX_LINE_LENGTH, asm_file) != NULL; cur_file_info.cur_line_number++) {
        if (strchr(cur_line, '\n') == NULL && !feof(asm_file)) { /* new line char not found, means line contains more chars than allowd */
            ...
            if (!validate_command(cur_file_info, s_table, &ic, &dc)) { /* means the line is not a valid instruction */
                if (is_success)
                    is_success = FALSE;
            }
    ...
    if (is_success) { /* means first pass was ok and we continue to second pass */
        ...
        tmp = s_table;
        while (tmp != NULL) {
            if(exists(s_table, tmp->name, DATA_SYMBOL))
                (tmp->address) += ic;
            tmp = tmp->next;
        }
