...
static bool assemble_r_instruction_group_1(file_info f_info, int i, opcode op, funct f, memory_cell *ic_memory_table, long *ic);
/* this function takes r instruction(copying) and tries to assemble it */
static bool assemble_r_instruction_group_2(file_info f_info, int i, opcode op, funct f, memory_cell *ic_memory_table, long *ic);
...
static bool assemble_r_instruction(file_info f_info, int i, opcode op, funct f, memory_cell *ic_memory_table, long *ic, int expected_args);
...
static bool assemble_i_instruction_group_1(file_info f_info, int i, opcode op, memory_cell *ic_memory_table, long *ic);
/* this function takes i instruction(jumping) and tries to assemble it */
static bool assemble_i_instruction_group_2(file_info f_info, int i, opcode op, memory_cell *ic_memory_table, symbol_table *s_table, long *ic);
/* this function takes i instruction(loading and saving in memory) and tries to assemble it */
static bool assemble_i_instruction_group_3(file_info f_info, int i, opcode op, memory_cell *ic_memory_table, long *ic);
...
static void build_i_instruction(memory_cell *ic_memory_table, opcode op, int rs, int rt, long immed);

/* this function takes jmp instruction and tries to assemble it */
static bool assemble_jmp_instruction(file_info f_info, int i, opcode op, memory_cell *ic_memory_table, symbol_table *s_table, extern_node *extern_list, long *ic);
/* this function takes la instruction and tries to assemble it */
static bool assemble_la_instruction(file_info f_info, int i, opcode op, memory_cell *ic_memory_table, symbol_table *s_table, extern_node *extern_list, long *ic);
/* this function takes call instruction and tries to assemble it */
static bool assemble_call_instruction(file_info f_info, int i, opcode op, memory_cell *ic_memory_table, symbol_table *s_table, extern_node *extern_list, long *ic);
/* this function takes j instruction(that accepts label only) and tries to assemble it */
static bool assemble_j_instruction_label_only(file_info f_info, int i, memory_cell *ic_memory_table, symbol_table *s_table, extern_node *extern_list, opcode op, long *ic);
/* this function takes stop instruction and tries to assemble it */
static bool assemble_stop_instruction(memory_cell *ic_memory_table, opcode op, long *ic);
/* this function takes details about j instruction and convert it to machine code */
static void build_j_instruction(memory_cell *ic_memory_table, opcode op, int reg, long address);

/* this function takes db instruction and tries to assemble it */
static bool assemble_db_instruction(file_info f_info, int i, memory_cell *dc_memory_table, long *dc);
/* this function takes dh instruction and tries to assemble it */
static bool assemble_dh_instruction(file_info f_info, int i, memory_cell *dc_memory_table, long *dc);
/* this function takes dw instruction and tries to assemble it */
static bool assemble_dw_instruction(file_info f_info, int i, memory_cell *dc_memory_table, long *dc);
...
static bool assemble_entry_instruction(file_info f_info, int i, symbol_table *s_table);
...
bool assemble_command(file_info f_info, memory_cell *ic_memory_table, memory_cell *dc_memory_table, symbol_table *s_table, extern_node *extern_list, long *ic, long *dc){
	...
	switch(instruct_info->id){
		...
		case BNE: return assemble_i_instruction_group_2(f_info, i, instruct_info->opcode, ic_memory_table, s_table, ic);		
		case BEQ: return assemble_i_instruction_group_2(f_info, i, instruct_info->opcode, ic_memory_table, s_table, ic); 
		case BLT: return assemble_i_instruction_group_2(f_info, i, instruct_info->opcode, ic_memory_table, s_table, ic); 
		case BGT: return assemble_i_instruction_group_2(f_info, i, instruct_info->opcode, ic_memory_table, s_table, ic); 
		...
		case JMP: return assemble_jmp_instruction(f_info, i, instruct_info->opcode, ic_memory_table, s_table, extern_list, ic);
		case LA: return assemble_la_instruction(f_info, i, instruct_info->opcode, ic_memory_table, s_table, extern_list, ic);
		case CALL: return assemble_call_instruction(f_info, i, instruct_info->opcode, ic_memory_table, s_table, extern_list, ic);
		...
		case ENTRY: return assemble_entry_instruction(f_info, i, s_table);
		case EXTERN: return assemble_extern_instruction(f_info, i, s_table);
...
static void build_r_instruction(memory_cell *ic_memory_table, opcode op, int funct, int rs, int rt, int rd){
    ...
    add_memory_cells(ic_memory_table, 4, value); /* converting the machine code into cells */
}
...
static bool assemble_i_instruction_group_2(file_info f_info, int i, opcode op, memory_cell *ic_memory_table, symbol_table *s_table, long *ic){
    ...
    while(!end_of_line(f_info.cur_line_content, i)){
        ...
        if(args_handled == 2){ /* means the current operand should be a label */
            if (!exists(s_table, label, ANY_SYMBOL)) { /* label does not exist */
                fprintf(stderr, "%s:%ld: label doesn't exist\n", f_info.name, f_info.cur_line_number);
                return FALSE;
            }

            if(exists(s_table, label, EXTERNAL_SYMBOL)){ /* label is external(not allowd in this instruction) */
                fprintf(stderr, "%s:%ld: label can't be external\n", f_info.name, f_info.cur_line_number);
                return FALSE;
            }
        }
    ...
    immed = get_address_of(s_table, label) - *ic; /* calculating the distance between the current memory location to the label location in memory */
...
static bool assemble_jmp_instruction(file_info f_info, int i, opcode op, memory_cell *ic_memory_table, symbol_table *s_table, extern_node *extern_list, long *ic) {
    ...
    if (label[0] == '$') { /* means the operand is a register */
        ...
        if (!exists(s_table, label, ANY_SYMBOL)) { /* means the label doesn't exist */
            fprintf(stderr, "%s:%ld: label doesn't exist\n", f_info.name, f_info.cur_line_number);
            return FALSE;
        }
        address = get_address_of(s_table, label); /* getting the address of the label */
    }

    if (exists(s_table, label, EXTERNAL_SYMBOL)) /* we saw an external label, we should save this for later processing */
        add_extern_item(extern_list, label, *ic); /* saving the location */
...
/* this function takes la instruction and tries to assemble it */
static bool assemble_la_instruction(file_info f_info, int i, opcode op, memory_cell *ic_memory_table, symbol_table *s_table, extern_node *extern_list, long *ic){
    return assemble_j_instruction_label_only(f_info, i, ic_memory_table, s_table, extern_list, op, ic); /* la instruction */
}

/* this function takes call instruction and tries to assemble it */
static bool assemble_call_instruction(file_info f_info, int i, opcode op, memory_cell *ic_memory_table, symbol_table *s_table, extern_node *extern_list, long *ic){
    return assemble_j_instruction_label_only(f_info, i, ic_memory_table, s_table, extern_list, op, ic); /* call instruction */
}
...
static bool assemble_j_instruction_label_only(file_info f_info, int i, memory_cell *ic_memory_table, symbol_table *s_table, extern_node *extern_list, opcode op, long *ic){
    ...
    build_j_instruction(ic_memory_table, op, 0, get_address_of(s_table, label)); /* converting the instruction to machine code */
...
static bool assemble_num_creation_instruction(file_info f_info, int i, memory_cell *dc_memory_table, long *dc, int cells_amount){
    ...
    while(!end_of_line(f_info.cur_line_content, i)){
        ...
        add_memory_cells(dc_memory_table, cells_amount, get_num(num)); /* adding the number we read to the memory */
...
static bool assemble_asciz_instruction(file_info f_info, int i, memory_cell *dc_memory_table, long *dc){
    ...
    add_memory_cells(dc_memory_table, 1, '\0'); /* string terminator */
    *dc += 1; /* a char takes one cell in memory */
...
static bool assemble_entry_instruction(file_info f_info, int i, symbol_table *s_table){
    ...
    if (exists(s_table, label, EXTERNAL_SYMBOL)) { /* means the label is external */
        fprintf(stderr, "%s:%ld: external label can't be entry\n", f_info.name, f_info.cur_line_number);
        return FALSE;
    }

    add_attributes(s_table, label, ENTRY_SYMBOL); /* means that from now on the label is entry */
